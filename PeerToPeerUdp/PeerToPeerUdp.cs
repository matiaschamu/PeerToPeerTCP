...
namespace PeerToPeer.Udp
{
	public partial class PeerToPeerUdp : Component
	{
		...
		public String Status()
		{
			...
			mT = mT + "Escuchando: " + this.EstadoListen.ToString() + "\r\n";
		...
		public void ListenStop()
		{
			try
			{
				...
				if (mTokenSourceCancelmTareaEscuchaUdp != null)
				{
					if (mTokenSourceCancelmTareaEscuchaUdp.IsCancellationRequested == false)
					{
		...
		private void PonerAEscucharElSock()
		{
			...
			//else
			//{
			if (mEstadoListen == eEstadosListen.Cerrado)
			{
				//mListenPedidoUsuario = true;
		...
		private void CrearSubProcesoEnvio()
		{
			...
			//if (ElSubProcesoEnvioUdpEstaEjecutando() == false)
			//{
		...
		//private bool ElSubProcesoEnvioUdpEstaEjecutando()
		//{
		...
		private void SubProcesoEnvioUdp()
		{
			try
			{
				...
				while (mTokenCancelmTareaEnvioUdp.IsCancellationRequested == false)
				{
		...
		private void SupProcesoEscuchaUdp()
		{
			...
			try
			{
				if (mSockUdpReceive == null)
				{
					mSockUdpReceive = new UdpClient();
					mSockUdpReceive.Connect(IPAddress.Any, PuertoEscucha);
				}
				mEstadoListen = eEstadosListen.Escuchando;
